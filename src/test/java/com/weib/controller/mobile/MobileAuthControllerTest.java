...
class MobileAuthControllerTest {
    ...
    private SessionRegistryService sessionRegistry;
    private MockMvc mvc;
    ...
    void setUp() {
        ...
        sessionRegistry = mock(SessionRegistryService.class);
        ...
        mvc = MockMvcBuilders.standaloneSetup(
                new MobileAuthController(userService, captchaService, jwtUtil, identityService, sessionRegistry)
        ).build();
    }
    ...
    void returnsBearerTokenForSeeker() throws Exception {
        ...
        when(jwtUtil.generateToken(eq(7L), eq("seeker_ahua"), eq("SEEKER"), anyString(), eq("MOBILE"))).thenReturn("seeker.jwt");
    ...
    void returnsBearerTokenForBoss() throws Exception {
        ...
        when(jwtUtil.generateToken(eq(8L), eq("boss_zhang"), eq("BOSS"), anyString(), eq("MOBILE"))).thenReturn("boss.jwt");
    ...
    void dualRoleAccountUsesSelectedBossIdentity() throws Exception {
        ...
        when(jwtUtil.generateToken(eq(7L), eq("seeker_ahua"), eq("BOSS"), anyString(), eq("MOBILE"))).thenReturn("boss.identity.jwt");
